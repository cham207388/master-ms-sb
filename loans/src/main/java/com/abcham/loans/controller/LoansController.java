...
import com.abcham.loans.dto.ErrorResponseDto;
import com.abcham.loans.dto.LoansDto;
import com.abcham.loans.dto.ResponseDto;
...
/**
 * @author Secured Bytes
 */

@Tag(name = "CRUD REST APIs for Loans in SecuredBank", description = "CRUD REST APIs in SecuredBank to CREATE, UPDATE, FETCH AND DELETE loan details")
@RestController
...
public class LoansController {
    ...
    @Operation(summary = "Create Loan REST API", description = "REST API to create new loan inside SecuredBank")
    @ApiResponses({
    ...
    public ResponseEntity<LoansDto> fetchLoanDetails(
            @RequestHeader("securedbank-correlation-id") String correlationId,
            @RequestParam @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits") String mobileNumber) {

        log.info("Correlation id from mobile: {}, {}", correlationId, mobileNumber);
        LoansDto loansDto = iLoansService.fetchLoan(correlationId, mobileNumber);
