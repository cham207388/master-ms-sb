...
9. **Observability Telemetry Stack** ([`docker/compose.observability.yml`](file:///Users/baicham/develop/java-projects/master-ms-sb/docker/compose.observability.yml), [`/observability`](file:///Users/baicham/develop/java-projects/master-ms-sb/observability); kind via [`helm/securedbank`](file:///Users/baicham/develop/java-projects/master-ms-sb/helm/securedbank) or raw [`kubernetes/11_loki.yml`](file:///Users/baicham/develop/java-projects/master-ms-sb/kubernetes/11_loki.yml)–[`15_prometheus.yml`](file:///Users/baicham/develop/java-projects/master-ms-sb/kubernetes/15_prometheus.yml))
 - **Grafana UI Port**: `3000`
 - **Loki Gateway Port**: `3100` (Read target: `3101`, Write target: `3102`) — Compose; Helm/raw kind use SingleBinary Service `loki:3100`
 - **MinIO S3 Store Ports**: `9000` (API) / `9001` (Console) — Compose only
 - **Grafana Alloy Port**: `12345`
 - **Domain**: Centralized log harvesting and visualization. Compose: Alloy reads Docker socket → Loki gateway (`tenant1`) + MinIO. Kind: Alloy DaemonSet → `loki:3100`; Tempo OTLP `tempo:4317`; Prometheus scrapes `/actuator/prometheus` (`make helm-up` or `make k8s-observability`).
...
- **Containerization & Orchestration**:
  ...
  - **Kubernetes (kind)**: Platform manifests in [`kubernetes/`](file:///Users/baicham/develop/java-projects/master-ms-sb/kubernetes) (`1_keycloak.yml`, `2_configmap.yml`); per-service objects under `<service>/k8s/` (`deployment.yml`, `service.yml`, and for domain APIs `db.yml` + `networkpolicy.yml`; message worker under [`message/k8s/`](file:///Users/baicham/develop/java-projects/master-ms-sb/message/k8s)). Observability: `kubernetes/11`–`15` (`make k8s-observability`). Optional DRY path: umbrella Helm chart [`helm/securedbank/`](file:///Users/baicham/develop/java-projects/master-ms-sb/helm/securedbank) (`make helm-up`) — Keycloak via codecentric/keycloakx + first-party `keycloak-db`, plus observability subcharts (Loki, Alloy, Tempo, Grafana, Prometheus); realm via [`infra/`](file:///Users/baicham/develop/java-projects/master-ms-sb/infra). Guide: [`docs/kubernetes.md`](file:///Users/baicham/develop/java-projects/master-ms-sb/docs/kubernetes.md).
...
2. **Makefile Commands**:
   - Use the root [`Makefile`](file:///Users/baicham/develop/java-projects/master-ms-sb/Makefile) targets for multi-service operations:
     ...
     - Kubernetes (kind): `make k8s-keycloak`, `make k8s-configmap`, `make k8s-calico` (NetworkPolicy CNI), `make k8s-kafka`, `make k8s-accounts` / `k8s-cards` / `k8s-loans` / `k8s-message`, `make k8s-config-server` / `k8s-eureka-server` / `k8s-gateway-server`, `make k8s-platform`, `make k8s-services`, `make k8s-up`, `make k8s-observability`; Helm: `make helm-up` / `helm-down` / `helm-lint` / `helm-template` (umbrella includes Keycloak + observability) — see [`docs/kubernetes.md`](file:///Users/baicham/develop/java-projects/master-ms-sb/docs/kubernetes.md)
