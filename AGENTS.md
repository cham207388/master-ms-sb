...
9. **Observability Telemetry Stack** ([`docker/compose.observability.yml`](file:///Users/baicham/develop/java-projects/master-ms-sb/docker/compose.observability.yml), [`/observability`](file:///Users/baicham/develop/java-projects/master-ms-sb/observability); kind via [`helm/securedbank`](file:///Users/baicham/develop/java-projects/master-ms-sb/helm/securedbank))
   - **Grafana UI Port**: `3000`
   - **Loki Gateway Port**: `3100` (Read target: `3101`, Write target: `3102`) — Compose; Helm uses SingleBinary Service `loki:3100`
   - **MinIO S3 Store Ports**: `9000` (API) / `9001` (Console) — Compose only
   - **Grafana Alloy Port**: `12345`
   - **Domain**: Centralized log harvesting and visualization. Compose: Alloy reads Docker socket → Loki gateway (`tenant1`) + MinIO. Helm: Alloy DaemonSet → `loki:3100`; Tempo OTLP `tempo:4317`; Prometheus scrapes `/actuator/prometheus`.
...
- **Containerization & Orchestration**:
  ...
  - **Kubernetes (kind)**: Platform manifests in [`kubernetes/`](file:///Users/baicham/develop/java-projects/master-ms-sb/kubernetes) (`1_keycloak.yml`, `2_configmap.yml`); per-service objects under `<service>/k8s/` (`deployment.yml`, `service.yml`, and for domain APIs `db.yml` + `networkpolicy.yml`; message worker under [`message/k8s/`](file:///Users/baicham/develop/java-projects/master-ms-sb/message/k8s)). Optional DRY path: umbrella Helm chart [`helm/securedbank/`](file:///Users/baicham/develop/java-projects/master-ms-sb/helm/securedbank) (`make helm-up`) — Keycloak via codecentric/keycloakx + first-party `keycloak-db`, plus observability subcharts (Loki, Alloy, Tempo, Grafana, Prometheus); realm via [`infra/`](file:///Users/baicham/develop/java-projects/master-ms-sb/infra). Guide: [`docs/kubernetes.md`](file:///Users/baicham/develop/java-projects/master-ms-sb/docs/kubernetes.md).
